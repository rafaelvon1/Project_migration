...
if __name__ == "__main__":
    ...
    upsert = UpSert()
    data_insert = upsert.insert
    data_update = upsert.update
    data_delete = upsert.delete
    ...
    for table in DATA["source_tables"]:
        try:
            ...
            pk = DATA["source_tables"][table]["column_pk"]
        ...
        try:
            ...
            raise e
        try:
            #df origem e df destino
            
            df_insert = data_insert(df,df_sql)
            df_update = data_update(df, df_sql)
            df_delete = data_delete(df, df_sql)
            df_delete = df_delete[pk].astype(int).tolist()
            with engine_sql.begin() as conn:
                if df_insert.empty:
                    print(f"Nenhum dado para inserir na tabela {table_name_destino}")
                else:
                    conn_sql.Save(df_insert, conn, table_name_destino, schema_destino)
                    print(f"Dados da tabela {table_name_destino} inseridos no SQL Server")
                if df_update.empty:
                    print(f"Nenhum dado para atualizar na tabela {table_name_destino}")
                else:
                    conn_sql.Update_where(pk,df_update, conn, table_name_destino, schema_destino)
                    print(f"Dados da tabela {table_name_destino} atualizados no SQL Server")
                if not df_delete:
                    print(f"Nenhum dado para deletar na tabela {table_name_destino}")
                else:
                    conn_sql.Delete_where(pk,df_delete, conn, table_name_destino, schema_destino)
                    print(f"Dados da tabela {table_name_destino} deletados no SQL Server")
            
            logger.info(f"upsert da tabela {table_name_destino} Realizado: \n\nDados Deletados:{len(df_delete)}\nDados Atualizados: {len(df_update)}\nDados Inseridos: {len(df_insert)}")
            print(f"upsert da tabela {table_name_destino} Realizado com sucesso para o SQL Server")
        except Exception as e:
            logger.error(f"Erro no processo de upsert da tabela {table_name_destino} no SQL Server: {e}")
            print(f"Erro no processo de upsert da tabela {table_name_destino} no SQL Server: {e}")
